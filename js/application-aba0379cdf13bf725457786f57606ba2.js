...
;
$(function () {
  $(document).on("click", "a[data-ajax-modal-id]", function(evt) {
    evt.preventDefault();
    var $a = $(evt.currentTarget)
      , href = $a.attr("href")
      , $modal = $("#" + $a.data("ajax-modal-id"));
...
$(function() {
  ...
  $("body").on("click", selector, function() {
    ...
    if (!isVisible) {
      $this.
        trigger($.Event("show.js-dropdown")).
        addClass("active").
        siblings("ul").
        show();
    }
...
var AutoShowModal = {
  check: function() {
    var $modal = $('[data-auto-show-modal]');
...
$(function() {
  $(document).on("click", ".js-ellipsis-toggle", function() {
    var $btn = $(this)
      , $targetElement = $($btn.data("toggle-target"));

    $btn.trigger($.Event("cc-ellipsis-toggle", {
      toggleElement: $targetElement[0],
      isExpanded: !$btn.hasClass("btn-depressed")
    }));
  });
});
$(function() {
  $(".smells").on("show.js-dropdown", ".js-dropdown > a", function(evt) {
    var $dropdownLink = $(evt.currentTarget)
      , $dropdown = $dropdownLink.siblings("ul")
      , aPosition = $dropdownLink.position();
    $dropdownLink.siblings("ul").css({
      top: aPosition.top + $dropdownLink.outerHeight() - 5,
      left: aPosition.left - $dropdown.outerWidth() + $dropdownLink.outerWidth()
    });
  });
});
$(function() {
  $("[data-dismiss-inferred-config]").click(function(event) {
    var $target = $(event.currentTarget);
    var $parent = $target.parent(".small_banner");
    var dismissUrl = $target.data("dismiss-url");

    $.ajax({ type: "DELETE", url: dismissUrl });
    $parent.slideUp();
  });
});
/**
...
/* http://prismjs.com/download.html?themes=prism-coy&languages=markup+css+clike+javascript+apacheconf+applescript+bash+c+csharp+cpp+coffeescript+css-extras+dart+diff+erlang+fsharp+git+go+haml+handlebars+haskell+http+ini+jade+java+julia+latex+less+markdown+matlab+objectivec+pascal+perl+php+php-extras+powershell+python+r+jsx+ruby+rust+sass+scss+scala+scheme+smalltalk+sql+swift+yaml */
...
;
// From https://github.com/jeluard/prism-clojure/blob/master/prism.clojure.js
